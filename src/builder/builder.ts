...
export default class Builder<T> {
  ...
  relationship(type: string | string[], direction?: Direction, alias?: string | null, properties?: object | undefined, degrees?: number | string): Builder<T> {
    const props = Object.entries(properties || {}).map(([key, value]) => new Property(key, this.aliasProperty(null, key, value).getParam()))
  ...
  whereId(alias: string, id: number): Builder<T> {
    this.currentStatement().whereId(alias, this.aliasProperty(alias, "_id", id).getParam())

    return this
  }

  whereNotId(alias: string, id: number): Builder<T> {
    this.currentStatement().whereNotId(alias, this.aliasProperty(alias, "_id", id).getParam())
