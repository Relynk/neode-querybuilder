...
import { Condition, Direction, Operator, Order, Prefix, SetOperator, StatementPrefix } from "../constants"
...
export default class Statement<T> {
  ...
  toString(): string {
    ...
    if (this.pattern.length) {
      output.push(`${this.prefix} ${this.pattern.map(value => value.toString()).join("")}`)
    }
